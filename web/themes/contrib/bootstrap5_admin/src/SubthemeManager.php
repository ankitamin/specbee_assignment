...
class SubthemeManager {
  ...
  public function createSubtheme(string $themeMName, string $themeName, string $subthemePathValue,) {
    ...
    $pathParent = \Drupal::service('extension.list.theme')->getPath($parentName) . DIRECTORY_SEPARATOR;
    // Create subtheme.
    if (empty($themeName)) {
    ...
    if (!is_dir($themePath)) {
      ...
      foreach ($subforders as $subforder) {
        ...
        $files = $fs->scanDirectory($pathParent . $subforder . DIRECTORY_SEPARATOR, '/.*/');
        foreach ($files as $file) {
          $fileName = $file->filename;
          $fs->copy($pathParent . $subforder . DIRECTORY_SEPARATOR . $fileName,
